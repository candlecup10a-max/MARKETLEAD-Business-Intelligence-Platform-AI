...
import { CustomerDemand, CompanyProfile, DemandStatus, Proposal, MessageLog, A2ANegotiationStep } from '../types';
...
interface DemandDetailModalProps {
  ...
  isOpen: boolean;
  initialTab?: 'overview' | 'email' | 'whatsapp' | 'a2a' | 'chat' | 'proposal';
  onClose: () => void;
...
export const DemandDetailModal: React.FC<DemandDetailModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'email' | 'whatsapp' | 'a2a' | 'chat' | 'proposal'>(initialTab);
  ...
  const [showPayloadStep, setShowPayloadStep] = useState<number | null>(null);
            ...
            <div className="space-y-4">
              <div className="bg-white p-4 rounded-2xl border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs">
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                      Autonomous Machine-to-Machine (A2A) Engine
                    </span>
                  </div>
                  <div className="text-xs text-slate-500">
                    Vendor Agent <code className="text-teal-700 bg-teal-50 px-1.5 py-0.5 rounded font-mono font-semibold">{companyProfile.a2aAgentId || 'A2A-VENDOR-770'}</code> ⇄ Buyer Agent <code className="text-cyan-800 bg-cyan-50 px-1.5 py-0.5 rounded font-mono font-semibold">{demand.a2aAgentId}</code>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-700 hover:to-teal-700 text-white rounded-xl text-xs font-bold flex items-center space-x-2 shadow-xs disabled:opacity-50 cursor-pointer"
                >
                  <Bot className={`h-4 w-4 ${isA2ARunning ? 'animate-spin' : ''}`} />
                  <span>{isA2ARunning ? 'Executing Handshake...' : 'Run Autonomous A2A Negotiation'}</span>
                </button>
              </div>

              {/* Negotiation Sequence Log */}
              {a2aSteps.length > 0 ? (
                <div className="space-y-3">
                  {a2aSteps.map((step, idx) => {
                    ...
                    return (
                      <div
                        key={`a2a-step-${idx}-${step.stepNumber ?? ''}-${step.action ?? ''}`}
                      ...
                      >
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs font-bold text-slate-900">
                              {isVendor ? 'Our Vendor AI' : `${demand.customerCompany} Buyer AI`}
                            </span>
                            <span className="text-[10px] font-mono text-slate-500 bg-white/80 px-1.5 py-0.5 rounded">
                              {step.action}
                            </span>
                          </div>
                        ...
                        <p className="text-xs text-slate-700 leading-relaxed pl-2 border-l-2 border-teal-500/50">
                          {step.message}
                        </p>

                        {step.payload && (
                          <div className="mt-2 text-[11px] font-mono bg-slate-900 text-emerald-400 p-2.5 rounded-xl overflow-x-auto">
                ...
                <div className="text-center py-12 bg-white rounded-2xl border border-slate-200 p-6 shadow-xs">
                  <Bot className="h-10 w-10 text-cyan-600/50 mx-auto mb-2" />
                  <p className="text-xs text-slate-800 font-semibold">No active A2A session executed yet</p>
                  <p className="text-[11px] text-slate-500 max-w-sm mx-auto mt-1 mb-4">
                    Click the button above to autonomously connect your company AI Agent with the customer's procurement AI Agent.
                  </p>
                </div>
              )}
            </div>
